...
% RRP-0030 enrichment (v4): explicit claim typing on all five claims.
% Note for future editors: keep each claim's optional argument on a
% single line — pylatexenc drops multi-line bracket groups and the
% parser's inline-recovery path is single-line only.
\subsection*{Claim 1}
\begin{claim}[type=empirical, evidence=observation, confidence=0.75, rationale={Hand-annotation of 200 citation pairs from one unusually self-referential corpus mix (cs.DL/cs.AI/stat.ML); 91 percent inter-annotator agreement; consistent with the 25--40 percent background range in prior citation-function work}, labels={citation-graphs, knowledge-graphs, position, annotation-study}, datasets={rrxiv-corpus-200-citation-pair-sample}, title={Citation over-attribution}]
\label{claim:c1}
...
\subsection*{Claim 2}
\begin{claim}[type=empirical, evidence=experiment, confidence=0.85, rationale={47-query comparison against a KG baseline, independently replicated at 41/47; the coverage figure is sensitive to query-set selection}, labels={citation-graphs, knowledge-graphs, position}, datasets={scholarly-kg-47-query-suite}, title={Typed-edge query coverage}]
\label{claim:c2}
...
% v4: the 89% coverage result is evidence consistent with the
% whitepaper's queryability claim — typed supports/depends_on/
% contradicts edges admit the useful triage queries.
\supports{rrxiv:2605.00006:claim:c2}{rrxiv:2605.00001:claim:queryability}
% v4: c2's BibTeX-compatibility half generalises 00002's finding that
% citation managers ingest claim-graph edges as a typed-citation
% extension, from claim-level edges to paper-level typed citations.
\extendsclaim{rrxiv:2605.00006:claim:c2}{rrxiv:2605.00002:claim:c7}
...
\subsection*{Claim 3}
\begin{claim}[type=theoretical, evidence=argument, confidence=0.9, rationale={Structural argument from the definitions in the background section; corroborated by Wikidata's 2024 merge log (roughly 340k merges) against zero retractive DOI merges}, labels={citation-graphs, knowledge-graphs, position}, title={KG identity instability}]
\label{claim:c3}
...
\subsection*{Claim 4}
\begin{claim}[type=theoretical, evidence=argument, confidence=0.9, rationale={Structural argument about mutability commitments; the retraction exception is handled by overlay per rrxiv:2605.00007}, labels={citation-graphs, knowledge-graphs, position}, title={Append-only modulo retraction}]
\label{claim:c4}
...
% v4: c4's defence of append-only against the retraction objection
% relies on 00007's retraction-as-annotation overlay model.
\dependson{rrxiv:2605.00006:claim:c4}{rrxiv:2605.00007:claim:c1}
...
\subsection*{Claim 5}
\begin{claim}[type=computational, evidence=experiment, confidence=0.85, rationale={Round-trip CIR identity verified for the rrxiv corpus only; not exercised at arXiv scale}, labels={citation-graphs, knowledge-graphs, position, implementation}, title={Reference implementation}]
\label{claim:c5}
...
% v4: the round-trip + type-stripping BibTeX exporter is evidence
% consistent with 00002's citation-manager-compatibility claim,
% without depending on it.
\supports{rrxiv:2605.00006:claim:c5}{rrxiv:2605.00002:claim:c7}
...
% v4 enrichment note: considered a \contradicts edge to a sibling claim
% and declined — no registered claim in the corpus asserts that citation
% graphs are (or should be operated as) knowledge graphs; this paper's
% thesis contradicts a folk position, not a corpus claim. Forcing a
% \contradicts edge here would be dishonest encoding.
...
\item Albis-Burdige, B., Claude. (2026). \emph{The rrxiv whitepaper.} \texttt{rrxiv:2605.00001}. --- Protocol commitments motivating typed edges.
\item Albis-Burdige, B., Claude. (2026). \emph{The claim graph as a first-class artifact.} \texttt{rrxiv:2605.00002}. --- Claim-level counterpart of the paper-level typing in this paper.
\item Albis-Burdige, B., Claude. (2026). \emph{Retraction notices as first-class data.} \texttt{rrxiv:2605.00007}. --- Append-only-modulo-retraction model referenced in claim~\ref{claim:c4}.
\end{itemize}
\end{document}
